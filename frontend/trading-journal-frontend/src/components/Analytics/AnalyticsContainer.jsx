...
const AnalyticsContainer = ({ trades }) => {
  ...
  const calculateAnalytics = () => {
    ...
    closedTrades.forEach((trade) => {
      const month = new Date(trade.exit_time).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'short',
      })
    ...
    const aGrades = trades.filter(
      (t) => t.checklist_grade && t.checklist_grade.startsWith('A')
    ).length
    const bGrades = trades.filter(
      (t) => t.checklist_grade && t.checklist_grade.startsWith('B')
    ).length
    const cGrades = trades.filter(
      (t) => t.checklist_grade && t.checklist_grade.startsWith('C')
    ).length
    const dGrades = trades.filter(
      (t) => t.checklist_grade && t.checklist_grade.startsWith('D')
    ).length
